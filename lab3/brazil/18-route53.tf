############################################
# Hostgru Zone (optional creation)
############################################

# Explanation: A hostgru zone is like claiming Kashyyyk in DNS—names here become law across the galaxy.
...
resource "aws_s3_bucket_policy" "gru_alb_logs_policy01" {
  ...
  # NOTE: This is a skeleton. Students may negru to adjust for region/account specifics.
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
